...
The server should record the time and set the joined field. In addition a stream with objectRef set to "user:id" should be created with the createdBy field set to the user's id.

* *required parameters:* userName, email, password
* *optional parameters:* preferences, displayName
* *returns*: a json object containing at least userName, email, and displayName.
...
Permissions control who can see and edit a stream besides the creator.
