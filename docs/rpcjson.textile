...
h3. GET /user/:id/feeds, returns the list of events for all the streams that a user is subscribed to. 

h3. GET /user/:id/feed, to get a specific stream for a particular user, must specify a streamId

h2. shift

h3. POST /shift to create a shift, a corresponding stream object with an objectRef property set to "shift:id" should be created along with the shift creator set as an admin for this stream. 
